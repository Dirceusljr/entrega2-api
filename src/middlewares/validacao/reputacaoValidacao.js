...
const validacaoCriarReputacao = {
    [Segments.BODY]: Joi.object().keys({
        ...
        avaliacao: Joi.number().min(0).max(10).precision(1).required().messages(mensagensCustomizadas('avaliacao')),
...
const validacaoAtualizarReputacao = {
    [Segments.BODY]: Joi.object().keys({
        trocaId: Joi.string().min(2).max(800).messages(mensagensCustomizadas('trocaId')),
        avaliacao: Joi.number().min(0).max(10).precision(1).messages(mensagensCustomizadas('avaliacao')),
